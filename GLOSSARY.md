...
| **Gap / range notes** | hand-lettered annotations in the true left page margin of the Blue Blood Rating list (wide desktop only). **Gap note**: `≈N% gap` with an arrow pointing straight into the gap between two groupings (the rating drop across the boundary). **Range brace**: a curly brace spanning the **Blue Bloods** and **Blue Blood Contenders** groupings, labelled with the rating spread (top team minus base, e.g. `3.0% range`). Off by default — toggle with the ✎ in the margin left of the Blue Blood Rating tab |
| **Tier anchors** | the Contenders / Powers / Brands boundaries are pinned to a program in `build-data.mjs` (`TIER_LAST_TEAM` = Auburn / Minnesota / North Carolina State) because those breaks don't fall on a clean rating gap; Blue Bloods and the Fringe still come from gap detection (`tiers.mjs`) |
