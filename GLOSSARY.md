# Glossary

Shared names for the pieces, so feedback can point precisely.

## The model

| Term | What it is | Where it lives |
| --- | --- | --- |
| **Stat** (×10) | a single raw number — e.g. *All-Time Wins*, *Weeks in the AP Top 10* | `src/config/stats.ts` → `STATS` |
| **Criterion** (×5) | a pair of stats treated as one theme: **AP Poll**, **Wins**, **Championships**, **All-Americans**, **NFL Draft** | `src/config/stats.ts` → `CATEGORIES` (the AP-Poll criterion's internal key is `perception`) |
| **Stat percentile** | a stat's rank within all 130 FBS programs, 0–100 | computed in `scripts/build-data.mjs` |
| **Criterion score** | 0–100, the mean of a criterion's two stat percentiles | `team.critScore[criterion]` |
| **Blue Blood Rating** (a.k.a. **Rating**) | 0–100. Take a program's 10 stat percentiles, drop the single highest and single lowest, average the other 8 | `team.rating`, `team.ratingRank` |
| **Rating distribution** | the Rating turned into a z-score and drawn on a normal curve | the "Bell curve" view on `/` |

## Groupings

| Term | What it is |
| --- | --- |
| **Grouping** (a.k.a. **tier**) | which band a program sits in. Six of them, boundaries placed at natural gaps in the Rating list (`scripts/lib/tiers.mjs`): **Blue Bloods** · **Blue Blood Fringe** · **Blue Blood Adjacent** · **National Powers** · **National Brands** · **The Field** |
| **Grouping blurb** | the paragraph under each grouping heading in the ranked list | `src/config/ranking.ts` → `GROUPING_BLURB` |
| **Peer comparison** | on the team card: where a program leads / trails the other teams *near it* on each criterion | `team.peer.summary`, built in `scripts/build-data.mjs` |

## Trajectory

| Term | What it is |
| --- | --- |
| **Trajectory** | the arrow: **Ascending ▲** / **Holding –** / **Receding ▼**. Compares a program's most recent 20% of seasons with its whole prior history, in cross-program percentile position, across AP weeks, AP top-10 weeks, win rate, national titles and draft picks | `team.trend.dir` |
| **Trajectory note** | the short pointed phrase after an Ascending/Receding label — *needs the decades*, *a former power*, *still climbing*, *back from the wilderness*, *new money*, *off its peak*, *trading on history*, *a long slide*. Never shown for Holding | `team.note` |
| **Standard label** | `<Grouping> · <Trajectory>[ · <Trajectory note>]` — the muted line on each ranked-list row | `team.label.standard` |
| **Tagline** | the hand-written one-liner about a program's history / peak eras (fan-proud, no disputed claims) | `data/manual/blurbs.csv` → `team.label.personal` |

## Views & pages

| Term | What it is |
| --- | --- |
| **The Chart** | the AP-poll scatter specifically — weeks in the poll vs. weeks in the top 10 — on its own page (`/the-chart`). The shareable one |
| **The Blue Blood Ranking** | the `/` home: the grouped ranked list (default) with a Bell-curve alternate |
| **Criteria pages** | `/criteria/:key` — one criterion at a time, as a logo plot or a bell curve |
| **Team card** | the highlighted-team detail panel (rating, criterion bars, trajectory, peer comparison, tagline) — `src/components/TeamCard.tsx` |
| **Favourite / highlighted team** | the program you pick (or click); it gets a ring + name on every chart and the team card |
| **Marker** | a logo or a coloured **bubble** on a chart |

## Data

| Term | What it is |
| --- | --- |
| **as-played** wins | wins as they happened on the field (the default) |
| **NCAA official** wins | as-played minus NCAA-vacated wins (`data/manual/vacated_wins.csv`); the toggle |
| **The store** | `data/api/` + `data/season-records.csv` + `data/manual/` — everything the site is built from. `build-data.mjs` never hits the network |
| **Title selectors** | which national-championship selectors count toward the Championships criterion — currently AP, BCS, CFP, Historical. One line in `scripts/build-data.mjs` (`TITLE_SELECTORS`); `Coaches` split-title co-champions are on file but off by default |

See `data/manual/README.md` for the schema of every hand-curated file.
