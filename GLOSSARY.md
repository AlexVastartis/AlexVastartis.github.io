...
| Term | What it is |
| --- | --- |
| **The staging layer** | `data/staging/` — the entire data store, hand-maintained CSVs committed to the repo. `build-data.mjs` reads only this and never hits the network; no fetch step, no API key. `_staging_*` = numbers, `_blurb_*` = the site's narrative text, `conferences.json` = current conference per school. `npm run build:data` compiles staging → `public/data/` |
...
| **All-time wins** | sum of per-season rows in `_staging_wins.csv`. **1936→present and, where game-level records survive, 1869→1935** carry `source=history` rows. For the ~33 programs the NCAA "FBS Records" book publishes an all-time won-loss line for, one `source=ncaa` row (from `_staging_wins_ncaa.csv`) instead covers the pre-1936 era, sized so the all-time **official** total matches the book (e.g. Ohio State 990-337-53 through 2024) with the live season on top. A handful of programs with no game-level data and no book line use `source=ncaa-history` per-season rows. Early totals thin out before ~1905 and run below the book. |
