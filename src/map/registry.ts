...
export const MAP_BLURB: Record<string, string> = {
  ...
  'the logo toggle': 'Switches the masthead between Logo A (the chosen design and the site favicon) and Logo B (the earlier design). Also reachable as ?logo=alt / ?logo=default in the URL. Remove once the comparison is no longer wanted.',
  ...
  'the vacated wins toggle': 'NCAA official (default) vs. as played — strikes or re-adds the wins and national titles the NCAA vacated (USC 2004–05). Applies to the present day, every As Of year, and every column of the By Decade chart.',
  'the as played chip': 'Stands in for the hidden NCAA official / as played switch when ?wins=asPlayed is in the URL — shows the mode is on, with a ✕ back to NCAA official.',
  'the year picker': 'As of Now / a decade off-season (1950–2020). Picking a year rebuilds every rating and tier from only the seasons through that off-season.',
  ...
  'the show-the-math button': 'Opens the full-screen rating-math view — every stat, every percentile, the trim, the trajectory formula, and the same breakdown for every As Of snapshot.',
  'the rating math view': 'The full-screen audit view itself — the Blue Blood Rating at every point in time, most recent first, with the arithmetic spelled out per row.',
  'the decade chart': 'By Decade view — every program at every As Of point in time plus today, as logos on a rank-by-column line chart. Follows the conference filter (rows close up). Top-right: a Tiers switch (tinted tier bands, off by default) and an Expand button (full screen, Esc to leave). Hover a logo to trace that program; click to highlight it. The As Of picker, snapshot banner and team strip are hidden here.',
  'the method recap': 'Restates the rating formula and clarifies that every row below is a cumulative total through that season, not a single year.',
  'the definitions': 'Plain-language definitions of Rating and Percentile, the two numbers every row in the table is built from.',
