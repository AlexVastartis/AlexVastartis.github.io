...
| `all_americans.csv` | one consensus selection: `year,school,player,pos,consensus,unanimous,source` | All-Americans criterion (+ trajectory) | **One row per player** from the NCAA "Football Award Winners" record book ("Consensus All-Americans by School"). `consensus` is always `Yes`; `unanimous` (`Yes`/`No`) is the book's `(*)`, tracked from 1924. `build-data.mjs` aggregates to per-(school, season) counts and a per-year series for the trajectory. Team names canonicalised to `teams.csv`; ~21 newer programs with no selections fall back to `stats_summary.csv`. |
