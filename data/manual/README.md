...
| `all_americans.csv` | one consensus selection: `year,school,player,consensus,unanimous,source` | All-Americans criterion (+ trajectory) | **One row per player**, 1889–present; `consensus`/`unanimous` are `Yes`/`No`. `build-data.mjs` aggregates to per-(school, season) counts and a per-year series for the trajectory. Team names are canonicalised to `teams.csv`; a handful of newer programs with no selections fall back to `stats_summary.csv`. |
